...
export const AuthProvider = ({ children }: AuthProviderProps) => {
    ...
    const logout = useCallback(() => {
        ...
        if (!isAuthenticated) {
            toast("You are alredy Logged out Click to Login", { action: { label: "Login", onClick: () => setIsOpen(true) }, duration: 8000 });
            return;
        }
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (payload.exp) {
                ...
                if (timeLeft <= 0) {
                    logout();
                    toast("Session expired. Please log in again.", { action: { label: "Login", onClick: () => setIsOpen(true) }, duration: 8000 });
                    return;
                }
